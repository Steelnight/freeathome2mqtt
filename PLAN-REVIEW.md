> **2026-09-04 note (merge of this branch into trunk, post-WP0–WP12).** This review predates all
> twelve work packages. Its "Resolution status" and "Decisions taken" below describe design calls
> made *at review time*, against the plan as it stood at that point — not what actually shipped.
> Where implementation (documented WP-by-WP in `CLAUDE.md`) took a different, usually simpler,
> path, `docs/` now reflects the shipped behaviour and this file was left as historical record
> rather than rewritten. Concretely, against the "Decisions taken" list below:
> - **A1** (debounce) — implementation kept the **original reset-on-event debounce** (window
>   **50 ms**, exactly 2 writes for a clean 2 s drag), *not* the leading-edge throttle / 350 ms
>   window this review proposed. `bus/commands.py`'s `CommandCoalescer` docstring and
>   `bench_command_debounce` (WP7) are the source of truth.
> - **B1** (`primary:` profile key) — never added. `CommandSpec`/the profile schema have no such
>   field; the primary command/attribute is still just "the first one declared", undeclared.
> - **B2** (`on_zero:` hook) — never added. P-08 is handled by `range: [1, 100]` clamping alone
>   (`profiles/lighting.yaml`); there is no cross-command redirect mechanism.
> - **B3** (`--health` CLI flag) — never added. `cli.py` still has only `--check-config`,
>   `--dry-run`, `--discover`, `--capture`; the container `HEALTHCHECK` (WP12) runs
>   `--check-config`, a deliberate simplification recorded in `CLAUDE.md`'s WP12 entry.
> - **B4** (avoid `schema: json` for HA light) — implementation went the other way:
>   `homeassistant/components.py`'s `build_light` uses `"schema": "json"` directly, fixing the one
>   real bug this surfaced (`_encode_bool01` sending `"1"`/`"0"` instead of truthy/falsy) rather
>   than switching schemas. `docs/04 §6` describes the shipped mechanism.
> - **B5** (conditional coverage gate) — the underlying point (no `captured/*.json` exists yet)
>   is still true and `docs/10 §9` still says so.
> - **A2/A3/A4/C1–C7** — the still-applicable ones were folded into `docs/` directly; C6
>   (`scenesTriggered` not reaching ingress) remains real and unresolved, tracked in the
>   architecture diagram as-is rather than papered over here.
